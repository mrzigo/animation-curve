...
const Curvature = class {
  ...
  _initialize(params) {
    ...
    this.isStop = true;
    this.callbacks = {
      leftPosition: params.leftPosition, // сообщить наружу что мы достигли положение слева
      rightPosition: params.rightPosition, // достигли положения справа, это бывает полезно,
                     // когда по окончании анимации что-то должно произойти на странице (например запустить следующий слайд)
    };
    ...
    this._reset();
    params.complete(); // тут нам повезло, что фабрика работает не на асинхронной инициализации, по этому комплит достаточно выполнить тут
    // дополнительные проверки существования params.complete опускаю
  }
  ...
  _reset() {
    ...
    const curveSVG = `M ${50} ${50} ` +
      `C ${50},${200} ${250},${100} ${250},${250} ` // будем смещать [250,100] -> [50,100] и [250,250] -> [50,250]
  ...
  _nextPoint() {
    ...
    if (this.delta < -Math.abs(this.speed)) {
        this.speed *= -1;
        this.callbacks.leftPosition();
    }

    if (this.delta > 1) {
        this.speed *= -1;
        this.callbacks.rightPosition();
    }
